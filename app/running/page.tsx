...
export const metadata: Metadata = {
  title: 'Running - makiharu',
  description: 'ランニングについての記録',
};

export default function RunningPage() {
  const posts = getAllPosts('running');

  return (
    <div className="max-w-2xl mx-auto px-6 py-16">
      <div className="mb-16">
        <h1 className="text-3xl font-semibold mb-6 tracking-tight">Running</h1>
      ...
      {posts.length > 0 ? (
        <ul className="space-y-10">
          {posts.map((post) => (
            <li key={post.slug}>
              <Link href={`/running/${post.slug}`} className="group block">
                <article>
