...
export const metadata: Metadata = {
  title: 'Reading - makiharu',
  description: '読書についての記録',
};

export default function ReadingPage() {
  const posts = getAllPosts('reading');

  return (
    <div className="max-w-[760px] mx-auto px-6 md:px-8 py-16 md:py-20">
      <div className="mb-20 md:mb-24">
        <h1 className="text-2xl md:text-3xl font-normal mb-8 text-gray-900">Reading</h1>
      ...
      <section>
        {posts.length > 0 ? (
          <ul className="space-y-10 md:space-y-12">
            {posts.map((post) => (
              <li key={post.slug}>
                <Link href={`/reading/${post.slug}`} className="group block">
                <article>
